...
#import archi_helpers_l3 as archi

def createModel(patchSize, optimizer='Adam', learningRate=0.001):# only on linse 3!!!!!!!!!!!!
    l1_reg=0
    l2_reg=1e-6
    cnn = Sequential()
...
def fTrain(X_train, y_train, X_test, y_test, sOutPath, patchSize, batchSize=64, learningRate=0.001, iEpochs=299, CV_Patient=0):
    ...
    if CV_Patient!=0: model_name=model_name+ 'CV' + str(CV_Patient)+'_'#determine if crossValPatient is used...
    model_name= model_name+ str(int(patchSize[0,0])) + str(int(patchSize[0,1])) +'_lr_' + str(learningRate) + '_bs_' + str(batchSize)
...
def fPredict3D(X,y,  sModelPath, sOutPath, batchSize=64):
    ...
    sModelPath=sModelPath.replace(".mat", "") #_json.txt
    print(sModelPath)
    weight_name = sModelPath + '_weights.h5'
...
def fGridTrain(X_train, y_train, X_test, y_test, sOutPath, patchSize, CV_Patient=0):
    
    # grid search over parameters, enter loops here...
    fTrain(X_train, y_train, X_test, y_test, sOutPath, patchSize, iEpochs=300, CV_Patient=CV_Patient)
