...
def fUnpatch2D(prob_list, patchSize, patchOverlap, actualSize, iClass):
    ...
    for iIndex in range(0, prob_list.shape[0], 1):
        ...
        print(lMask.shape)
        print iCorner[0], iCorner[1], iCorner[2]
        ...
        iCorner[0] =int(iCorner[0]+dNotOverlap[0]) #iCorner[1] =int(iCorner[1]+dNotOverlap[1])
        print paddedSize[0],paddedSize[1]
        if iCorner[0] + patchSize[0] - 1 > paddedSize[0]: #switch 0 and 1
            iCorner[0] = 0                               # switch 0 and 1
            iCorner[1] = int(iCorner[1] + dNotOverlap[1]) # switch 0 and 1

        if iCorner[1] + patchSize[1] - 1 > paddedSize[1]:# switch 0 and 1
            iCorner[0] = 0# switch 0 and 1
            iCorner[1] = 0# switch 0 and 1
...
def fUnpatch3D(prob_list, patchSize, patchOverlap, actualSize, iClass):
    ...
    for iIndex in range(0, prob_list.shape[0], 1):
        print(iIndex)
        lMask = np.zeros((paddedSize[0], paddedSize[1], paddedSize[2]))
        lMask[iCorner[0]: iCorner[0] + int(patchSize[0]), iCorner[1]: iCorner[1] + int(patchSize[1]), iCorner[2]: iCorner[2] + int(patchSize[2])] = 1
        unpatchImg[iCorner[0]: iCorner[0] + int(patchSize[0]), iCorner[1]: iCorner[1] + int(patchSize[1]), iCorner[2]: iCorner[2] + int(patchSize[2])] = np.add(unpatchImg[iCorner[0]: iCorner[0] + int(patchSize[0]), iCorner[1]: iCorner[1] + int(patchSize[1]), iCorner[2]: iCorner[2] + int(patchSize[2])], prob_list[iIndex,iClass])
        lMask = lMask == 1
        numVal[lMask] = numVal[lMask] + 1
