...
cmap_selection = uicontrol('Parent',o_panel,...
    ...
    'FontSize',12,...
    'String',{'parula','parula2','jet','jet2','jet3','gray','autumn','bone',...
...
f_panel = copyobj(b_panel,f);set(f_panel,'Title','Foreground','Position',get(b_panel,'Position')+[deltaX+panelW,0,0,0]);
f_axes = findobj(f,'Type','axes','Parent',f_panel);
f_var = findobj(f_panel,'Tag','variable');
f_slice_edit = findobj(f_panel,'Tag','slice_edit');set(f_slice_edit,'Callback',@slice_edit_Callback);
f_frame_edit = findobj(f_panel,'Tag','frame_edit');set(f_frame_edit,'Callback',@frame_edit_Callback);
f_clim_low_edit = findobj(f_panel,'Tag','clim_low_edit');set(f_clim_low_edit,'Callback',@update_clim_Callback);
f_clim_high_edit = findobj(f_panel,'Tag','clim_high_edit');set(f_clim_high_edit,'Callback',@update_clim_Callback);
f_clim_lock = findobj(f_panel,'Tag','clim_checkbox');
f_slice_up = findobj(f_panel,'Tag','slice','String','>');set(f_slice_up,'Callback',@image_change_Callback);
f_slice_down = findobj(f_panel,'Tag','slice','String','<');set(f_slice_down,'Callback',@image_change_Callback);
f_frame_up = findobj(f_panel,'Tag','frame','String','>');set(f_frame_up,'Callback',@image_change_Callback);
f_frame_down = findobj(f_panel,'Tag','frame','String','<');set(f_frame_down,'Callback',@image_change_Callback);
h_f_im = findobj(f_panel,'Type','image');
set(findobj(f_panel,'Tag','variable'),'Callback',@variable_Callback);
