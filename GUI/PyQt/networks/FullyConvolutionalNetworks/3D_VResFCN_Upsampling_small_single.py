...
def createModel(patchSize, numClasses, usingClassification=False):
    ...
    if usingClassification:
        cnn = Model(inputs=[input_tensor],
                    outputs=[segmentation_output_foreground, segmentation_output_background, classification_output],
                    name='3D-VResFCN-Classification')
        sModelName = cnn.name
    else:
        cnn = Model(input_tensor, outputs=[segmentation_output_foreground, segmentation_output_background],
                    name='3D-VResFCN')
        sModelName = cnn.name
...
def fTrainInner(cnn, modelName, X_train=None, y_train=None, Y_segMasks_train=None, X_valid=None, y_valid=None,
    ...
    if usingClassification:
        cnn.compile(
            loss={'segmentation_output_foreground': dice_coef_loss, 'segmentation_output_background': dice_coef_loss,
                  'classification_output': 'sparse_categorical_crossentropy'},
            optimizer=opti,
            metrics={'segmentation_output_foreground': dice_coef, 'segmentation_output_background': dice_coef,
    ...
    if X_valid.size == 0 and y_valid.size == 0:
        ...
        if usingClassification:
            result = cnn.fit(X_train,
                             {'segmentation_output_foreground': np.expand_dims(Y_segMasks_train[:, :, :, :, 0],
                                                                               axis=-1),
                              'segmentation_output_background': np.expand_dims(Y_segMasks_train[:, :, :, :, 1],
                                                                               axis=-1),
                              'classification_output': y_train},
                             validation_data=(
                                 X_test, {
                                     'segmentation_output_foreground': np.expand_dims(Y_segMasks_test[:, :, :, :, 0],
                                                                                      axis=-1),
                                     'segmentation_output_background': np.expand_dims(Y_segMasks_test[:, :, :, :, 1],
                                                                                      axis=-1),
        ...
        if usingClassification:
            result = cnn.fit(X_train,
                             ...
                             validation_data=(
                                 X_valid, {
                                     'segmentation_output_foreground': np.expand_dims(Y_segMasks_valid[:, :, :, :, 0],
                                                                                      axis=-1),
                                     'segmentation_output_background': np.expand_dims(Y_segMasks_valid[:, :, :, :, 1],
                                                                                      axis=-1),
            ...
            result = cnn.fit(X_train,
                             {'segmentation_output_foreground': np.expand_dims(Y_segMasks_train[:, :, :, :, 0], axis=-1),
                              'segmentation_output_background': np.expand_dims(Y_segMasks_train[:, :, :, :, 1],
                                                                              axis=-1)},
                             validation_data=(
                                 X_valid, {
    ...
    if usingClassification:
        ...
        loss_test, segmentation_output_loss_test, classification_output_loss_test, segmentation_output_dice_coef_test, classification_output_acc_test \
            = cnn.evaluate(X_test, {'segmentation_output_foreground': np.expand_dims(Y_segMasks_test[:, :, :, :, 0],
                                                                                    axis=-1),
                                    'segmentation_output_background': np.expand_dims(Y_segMasks_test[:, :, :, :, 1],
                                                                                    axis=-1),
                                    'classification_output': y_test},
