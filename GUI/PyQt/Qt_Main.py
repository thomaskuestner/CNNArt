...
from config.PATH import DLART_OUT_PATH, PATH_OUT, MARKING_PATH, LABEL_PATH
...
class imagine( QtWidgets.QMainWindow, Ui_MainWindow ):
    ...
    def __init__(self, *args):
        ...
        # self.processObserver = QtWidgets.QMdiSubWindow()
        # self.processObserver.setGeometry( 1000, 250, 300, 400 )
        # self.processObserver.setWindowTitle( 'Proccess observer' )
        # self.processObserver.setWindowFlags( QtCore.Qt.WindowStaysOnTopHint )
        ...
        # self.text_plain.setPlainText(result_string)
        # self.processObserver.setWidget( self.text_plain )
        #
        # # # Signals and Slots
        #
        # self.actionProcess_Observer.triggered.connect(self.show_print)
    ...
    def load_MR(self, index):
        ...
        if type( self.PathDicom ) is str:
            ...
            self.array = self.PathDicom
            self.PathDicom = ''
            self.selectorPath = self.PathDicom
            self.newMR = loadImage( self.array )
        else:
            pass
    ...
    def load_patch(self, ind):
        ...
        if ind == 1:
            ...
            self.resultfile = dialog.getOpenFileName( self, "choose the result file",
                                                      MARKING_PATH,
                                                      'mat files(*.mat);;h5 files(*.h5);;npy files(*.npy);;npz files(*.npz)' )[
                0]
    ...
    def set_color(self):
        ...
        if self.selectoron:
            ...
            if ok:
                ...
                cmaps = cm
                vtrs = vs
                colormaplist[resultnum[0]] = cmaps
    ...
    def edit_markingfile(self, plist):
        ...
        with open( self.markfile, 'r' ) as json_data:
            ...
            if layer_name in saveFile['layer']:
                ...
                saveFile['layer'][layer_name][number_str] = {'vertices': plist, 'codes': None}
            else:
                numinkey = '001'
                number_str = ind + col_str + numinkey
                saveFile['layer'][layer_name] = {number_str: {'vertices': plist, 'codes': None}}
    ...
    def marking_shape(self, n):
        for item in self.view_group_list:
            ...
            self.canvasl.labelpath.connect(self.edit_markingfile)
                ...
                if self.radioButton_probabilites.isChecked():
                    ...
                    for i in range( len( self.multiclassCheckboxes ) ):
                        if bool( self.multiclassCheckboxes[i].isChecked() ):
                            ...
                            map = ax1.imshow( prediction_slice, cmap=self.multiclass_colormaps[i],
                    ...
                    im3 = ax1.contourf( np.squeeze( IArte[:, :, index] ), hatches=[None, '//', '\\\\', 'XX'],
    ...
    def plotArtifactMaps(self):
        ...
        if unpatches_slices is not None:
            ...
            if index >= 0 and index < dicom_slices.shape[-1]:
                ...
                map = ax2.imshow( prob_mask, cmap=self.artifact_colormap, interpolation='nearest', alpha=.4, vmin=0,
                                  vmax=1 )
    ...
    def button_train_clicked(self):
        ...
        try:
            ...
            raise ValueError(
                "Wrong input format of learning rates! Enter values seperated by ','. For example: 0.1,0.01,0.001" )
    ...
    def on_wyChooseFile_clicked(self):
        ...
        if len( self.openfile_name ) == 0:
            ...
            plot_model ( self.model, to_file='configGUI/model.png', show_shapes=True, show_layer_names=True )
...
if __name__ == '__main__':
    ...
    app = QtWidgets.QApplication ( sys.argv )
    # mainWindow = imagine('/home/yi/Desktop/Forschungsarbeit/WIP_YI/config/database/MRPhysics/newProtocol/01_ab/dicom_sorted/t1_tse_tra_fs_Becken_0008',
    #                       '/home/yi/Desktop/Forschungsarbeit/WIP_YI/config/database/MRPhysics/newProtocol/01_ab/dicom_sorted/t1_tse_tra_Kopf_Motion_0003')
    #
    # img1 = loadImage('/home/yi/Desktop/Forschungsarbeit/WIP_YI/config/database/MRPhysics/newProtocol/01_ab/dicom_sorted/t1_tse_tra_Kopf_Motion_0003')
    # img2 = loadImage('/home/yi/Desktop/Forschungsarbeit/WIP_YI/config/database/MRPhysics/newProtocol/01_ab/dicom_sorted/t1_tse_tra_Kopf_0002')
    # # 5 D array in the order: time channel column row slice
    # # 4 D array in the order: time column row slice
    # array1 = img1.voxel_ndarray
    # array2 = img2.voxel_ndarray
    # pics1 = [array1, array1, array1, array1]
    # pics1 = [np.expand_dims(pic, axis=0) for pic in pics1]
    # array4d1 = np.concatenate(pics1)
    # pics2 = [array2, array2, array2, array2]
    # pics2 = [np.expand_dims(pic, axis=0) for pic in pics2]
    # array4d2 = np.concatenate(pics2)
    # array5d = [array4d1, array4d2]
    # array5d = [np.expand_dims(pic, axis=0) for pic in array5d]
    # nimg = np.concatenate(array5d)
    # mainWindow = imagine(nimg)
    # Img5D = np.random.randn(4,5,32,64,10)
    mainWindow = imagine ()
    mainWindow.showMaximized ()
    mainWindow.show ()
    sys.exit ( app.exec_ () )