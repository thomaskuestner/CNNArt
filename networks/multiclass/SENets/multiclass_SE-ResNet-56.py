...
def createModel(patchSize, numClasses):
    ...
    x = identity_block(x, [16, 16], stage=1, block=6, se_enabled=True)
    x = identity_block(x, [16, 16], stage=1, block=7, se_enabled=True)
    x = identity_block(x, [16, 16], stage=1, block=8, se_enabled=True)
    x = identity_block(x, [16, 16], stage=1, block=9, se_enabled=True)
    ...
    x = identity_block(x, [32, 32], stage=2, block=6, se_enabled=True)
    x = identity_block(x, [32, 32], stage=2, block=7, se_enabled=True)
    x = identity_block(x, [32, 32], stage=2, block=8, se_enabled=True)
    x = identity_block(x, [32, 32], stage=2, block=9, se_enabled=True)
    ...
    x = identity_block(x, [64, 64], stage=3, block=6, se_enabled=True)
    x = identity_block(x, [64, 64], stage=3, block=7, se_enabled=True)
    x = identity_block(x, [64, 64], stage=3, block=8, se_enabled=True)
    x = identity_block(x, [64, 64], stage=3, block=9, se_enabled=True)
...
def fTrainInner(X_train, y_train, X_test, y_test, sOutPath, patchSize, batchSize=None, learningRate=None, iEpochs=None):
    ...
    model_name = sPath + '/' + sFilename + str(patchSize[0]) + str(patchSize[1]) + '_lr_' + str(
