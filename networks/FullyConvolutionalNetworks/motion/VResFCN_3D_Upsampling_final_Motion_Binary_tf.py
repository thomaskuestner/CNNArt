...
from networks.FullyConvolutionalNetworks.motion.deep_residual_learning_blocks_tf import *
...
def fTrain(datagenerator, X_test, y_test, Y_segMasks_test, sOutPath=None, patchSize=0, batchSize=None, learningRate=None, iEpochs=None, dlnetwork = None):
    ...
    numClasses = 2  # TODO: hard-coded, read from size of tensor

    #create cnn model
    print('Create model')
    cnn, model_name = createModel(patchSize=patchSize, numClasses=numClasses, usingClassification=usingClassification)
    ...
    if dlnetwork != None:
        if dlnetwork.optimizer == 'SGD':
            opti = tf.keras.optimizers.SGD(lr=learningRate,
                                        momentum=dlnetwork.momentum,
                                        decay=dlnetwork.weightdecay,
                                        nesterov=dlnetwork.nesterov)

        elif dlnetwork.optimizer == 'RMSPROP':
            opti = tf.keras.optimizers.RMSprop(lr=learningRate, decay=dlnetwork.weightdecay)

        elif dlnetwork.optimizer == 'ADAGRAD':
            opti = tf.keras.optimizers.Adagrad(lr=learningRate, epsilon=None, decay=dlnetwork.weightdecay)

        elif dlnetwork.optimizer == 'ADADELTA':
            opti = tf.keras.optimizers.Adadelta(lr=learningRate, rho=0.95, epsilon=None,
                                             decay=dlnetwork.weightdecay)

        elif dlnetwork.optimizer == 'ADAM':
            opti = tf.keras.optimizers.Adam(lr=learningRate, beta_1=0.9, beta_2=0.999, epsilon=None,
                                         decay=dlnetwork.weightdecay)
        else:
            raise ValueError("Unknown Optimizer!")
    else:
        # opti = SGD(lr=learningRate, momentum=1e-8, decay=0.1, nesterov=True);#Adag(lr=0.01, epsilon=1e-06)
        opti = tf.keras.optimizers.Adam(lr=learningRate, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)
    ...
    if X_valid == 0 and y_valid == 0:
        ...
        if usingClassification:
            result = cnn.fit(datagenerator[0],
                             {'segmentation_output': datagenerator[2], 'classification_output': datagenerator[1]},
...
def fPredict(X_test, Y_test=None, Y_segMasks_test=None, sModelPath=None, batch_size=64, usingClassification=False, usingSegmentationMasks=True, dlnetwork=None):
    ...
    if dlnetwork != None:
        if dlnetwork.optimizer == 'SGD':
            opti = tf.keras.optimizers.SGD(lr=dlnetwork.learningRate,
                                        momentum=dlnetwork.momentum,
                                        decay=dlnetwork.weightdecay,
                                        nesterov=dlnetwork.nesterov)

        elif dlnetwork.optimizer == 'RMSPROP':
            opti = tf.keras.optimizers.RMSprop(lr=dlnetwork.learningRate, decay=dlnetwork.weightdecay)

        elif dlnetwork.optimizer == 'ADAGRAD':
            opti = tf.keras.optimizers.Adagrad(lr=dlnetwork.learningRate, epsilon=None, decay=dlnetwork.weightdecay)

        elif dlnetwork.optimizer == 'ADADELTA':
            opti = tf.keras.optimizers.Adadelta(lr=dlnetwork.learningRate, rho=0.95, epsilon=None,
                                             decay=dlnetwork.weightdecay)

        elif dlnetwork.optimizer == 'ADAM':
            opti = tf.keras.optimizers.Adam(lr=dlnetwork.learningRate, beta_1=0.9, beta_2=0.999, epsilon=None,
                                         decay=dlnetwork.weightdecay)
        else:
            raise ValueError("Unknown Optimizer!")
    else:
        # opti = SGD(lr=learningRate, momentum=1e-8, decay=0.1, nesterov=True);#Adag(lr=0.01, epsilon=1e-06)
        opti = tf.keras.optimizers.Adam(lr=dlnetwork.learningRate, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)
