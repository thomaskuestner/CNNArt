...
def createModel(patchSize, numClasses, usingClassification=False):
    ...
    x_after_stage_1 = x #Add()([input_tensor, x])
    ...
    x_1 = Conv3D(filters=1,
               kernel_size=(1, 1, 1),
               strides=(1, 1, 1),
               padding='same',
               kernel_initializer='he_normal',
               name='conv_veryEnd_foreground')(x)

    x_2 = Conv3D(filters=1,
                 kernel_size=(1, 1, 1),
                 strides=(1, 1, 1),
                 padding='same',
                 kernel_initializer='he_normal',
                 name='conv_veryEnd_background')(x)

    segmentation_output_foreground = Softmax(axis=bn_axis, name='segmentation_output_foreground')(x_1)
    segmentation_output_background = Softmax(axis=bn_axis, name='segmentation_output_background')(x_2)

    # create model
    if usingClassification:
        cnn = Model(inputs=[input_tensor], outputs=[segmentation_output_foreground, segmentation_output_background, classification_output],
                    name='3D-VResFCN-Classification')
        sModelName = cnn.name
    else:
        cnn = Model(input_tensor, outputs=[segmentation_output_foreground, segmentation_output_background], name='3D-VResFCN')
        sModelName = cnn.name
...
def fTrain(X_train=None, y_train=None, Y_segMasks_train=None, X_valid=None, y_valid=None, Y_segMasks_valid=None,
    ...
    #intom = (patchSize[0], patchSize[1], patchSize[2], 1)
    #cnn = DenseNet3D(in_shape=intom, k=12, ls=[4, 4], theta=2, k_0=16, lbda=0, out_res=None, feed_pos=False, pos_noise_stdv=0)
    #sModelName = 'Shit CNN'
...
def fTrainInner(cnn, modelName, X_train=None, y_train=None, Y_segMasks_train=None, X_valid=None, y_valid=None,
    ...
    if usingClassification:
        ...
        cnn.compile(loss={'segmentation_output_background': dice_coef_loss, 'segmentation_output_foreground': dice_coef_loss},
                    optimizer=opti,
                    metrics={'segmentation_output_background': dice_coef, 'segmentation_output_foreground': dice_coef}
                    )
    ...
    if X_valid.size == 0 and y_valid.size == 0:
        ...
        if usingClassification:
            ...
            result = cnn.fit(X_train,
                             {'segmentation_output_foreground': Y_segMasks_train[:,:,:,:,1], 'segmentation_output_background': Y_segMasks_train[:,:,:,:,0]},
                             validation_split=0.15,
                             epochs=iEpochs,
                             batch_size=batchSize,
                             callbacks=callbacks,
        ...
        if usingClassification:
            ...
            result = cnn.fit(X_train,
                             {'segmentation_output_foreground': Y_segMasks_train[:, :, :, :, 1],
                              'segmentation_output_background': Y_segMasks_train[:, :, :, :, 0]},
                             validation_split=0.15,
                             epochs=iEpochs,
                             batch_size=batchSize,
                             callbacks=callbacks,
    ...
    if usingClassification:
        ...
        score_test, dice_coef_test = cnn.evaluate(X_test, {'segmentation_output_foreground': Y_segMasks_test[:,:,:,:,1], 'segmentation_output_background': Y_segMasks_test[:,:,:,:,0]}, batch_size=batchSize, verbose=1)
