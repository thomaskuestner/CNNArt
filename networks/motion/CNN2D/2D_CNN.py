...
def fTrain(X_train, Y_train, X_test, Y_test, sOutPath, patchSize, batchSize, learning_rate, iEpochs,\
           sInPaths=None,sInPaths_valid=None,CV_Patient=0, model='motion_head'):#rigid for loops for simplicity
    #add for loops here
    #learning_rate = 0.001
    cnn= fCreateModel(patchSize, learningRate=learning_rate, optimizer='Adam')
    print "Model: 2D_CNN"
    fTrainInner(cnn, X_train, Y_train, X_test, Y_test, sOutPath, patchSize, batchSize, learning_rate, iEpochs, CV_Patient=CV_Patient)
...
def fTrainInner(cnn, X_train, y_train, X_test, y_test, sOutPath, patchSize, batchSize=64, learningRate=0.001, iEpochs=299, CV_Patient=0):
    ...
    model_name = model_name + str(int(patchSize[0])) + str(int(patchSize[0])) + '_lr_' + str(learningRate) + '_bs_' + str(batchSize)
