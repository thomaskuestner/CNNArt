...
def encode(input, patchSize, bn=False):
...
def encode_shared(input, patchSize, bn=False):
    if list(patchSize) == [80, 80]:
        ...
        if bn:
            ...
            conv_shared_2 = fCreateLeakyReluBNConv2D(filters=256, strides=2, kernel_size=3)(conv_shared_1)
            # output shape: (256, 12, 12)
            conv_shared_3 = fCreateLeakyReluBNConv2D(filters=256, strides=1, kernel_size=3)(conv_shared_2)
            # output shape: (256, 6, 6)
            conv_shared_4 = fCreateLeakyReluBNConv2D(filters=256, strides=2, kernel_size=3)(conv_shared_3)
...
def decode(input, patchSize, bn=False):
    if list(patchSize) == [80, 80]:
        ...
        if bn:
            ...
            # output shape: (256, 12, 12)
            output = fCreateConv2DBNTranspose(filters=256, kernel_size=3, strides=2)(reshape)
            # output shape: (256, 12, 12)
            output = fCreateConv2DBNTranspose(filters=256, kernel_size=3, strides=1)(output)
            # output shape: (128, 24, 24)
            output = fCreateConv2DBNTranspose(filters=128, kernel_size=3, strides=2)(output)
            # output shape: (64, 24, 24)
            output = fCreateConv2DBNTranspose(filters=64, kernel_size=3, strides=1)(output)
            # output shape: (32, 48, 48)
            output = fCreateConv2DBNTranspose(filters=32, kernel_size=3, strides=2)(output)
            output = Conv2DTranspose(filters=1, kernel_size=1, strides=1, padding='same', activation='tanh')(output)
        else:
            dense = Dense(256 * 6 * 6)(input)
            # output shape: (256, 6, 6)
