...
def encode(input, patchSize, architecture='old'):
    if architecture == 'old':
        ...
        # output shape: (64, 24, 24)
        output = fCreateLeakyReluConv2D(filters=64, strides=2, kernel_size=3)(input)
        output = fCreateLeakyReluConv2D(filters=64, strides=1, kernel_size=3)(output)
        # output shape: (128, 12, 12)
        output = fCreateLeakyReluConv2D(filters=128, strides=2, kernel_size=3)(output)
        output = fCreateLeakyReluConv2D(filters=128, strides=1, kernel_size=3)(output)
        # otuput shape: (128, 6, 6)
        output = fCreateLeakyReluConv2D(filters=256, strides=2, kernel_size=3)(output)
        output = fCreateLeakyReluConv2D(filters=256, strides=1, kernel_size=3)(output)

    return output


def encode_shared(input, patchSize, architecture='old'):
    if architecture == 'old':
        if list(patchSize) == [80, 80]:
        ...
        if list(patchSize) == [48, 48]:
            # output shape: (128, 12, 12)
            for _ in range(3):
                output = fCreateConv2D_ResBlock(filters=256)(input)

            flat = Flatten()(output)

            z_mean = Dense(100)(flat)

            z_log_var = Dense(100)(flat)

            z = Lambda(samplingDense, output_shape=(100,))([z_mean, z_log_var])

    return z, z_mean, z_log_var


def decode(input, patchSize, dropout, architecture='old'):
    if architecture == 'old':
        if list(patchSize) == [80, 80]:
