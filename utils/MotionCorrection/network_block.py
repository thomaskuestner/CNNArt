...
def encode(input, patchSize):
    ...
    elif list(patchSize) == [48, 48, patchSize[2]]:
...
def decode(input, patchSize, conv_1, conv_2, conv_3, conv_4, arch):
    if arch == "vae-mocounet":
        if list(patchSize) == [48, 48]:
            ...
            dense = Dense(512 * 6 * 6 * patchSize[2])(input)
            # output shape: (512, 6, 6, patchSize[2])
            reshape = Reshape((512, 6, 6, patchSize[2]))(dense)
            # output shape: (256, 12, 12, patchSize[2])
            # output = fCreateConv3DTranspose_ResBlock(filters=512)(reshape)
            # output = add([output, conv_4])
            # output shape: (256, 20, 20, patchSize[2])
            output = fCreateConv3DTranspose_ResBlock(filters=256)(reshape)
            output = add([output, conv_3])
