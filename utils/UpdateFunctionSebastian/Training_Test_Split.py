...
#########################################################################################################################################
#module: DatasetSplit                                                                                                                   #
#The function DatasetSplit has the task to split the whole data in training data and validation data.                                   #
#########################################################################################################################################
def fSplitDataset(resultFolder, proband_list, model_list, allPatches, allY, sSplitting, patchSize, patchOverlap, split_ratio):
    ...
    if sSplitting == "normal_rand":
        ...
        Path = resultFolder + 'normal/Kopf_Leber_Move_05_label05_test_Becken_' +  str(patchSize[0]) + str(patchSize[1]) +'.h5'
        print(Path)
        with h5py.File(Path, 'w') as hf:
            hf.create_dataset('X_train', data=X_train)
            hf.create_dataset('X_test', data=X_test)
...
def fSplitDataset3D(resultFolder, proband_list, model_list, allPatches, allY, sSplitting, patchSize, patchOverlap, split_ratio):
    ...
    if sSplitting == "normal_rand":
        ...
        nPatches = allPatches.shape[0]
        print(allPatches.shape)
        dVal = 1800 #math.floor(split_ratio * nPatches)
        num_ind = np.arange(nPatches)
        ...
        Path = resultFolder + 'normal_3D/Lebert1_Move_05_label05_val_ab_test_ma_' +  str(patchSize[0]) + str(patchSize[1]) +'.h5'
        print(Path)
        with h5py.File(Path, 'w') as hf:
            hf.create_dataset('X_train', data=X_train)
            hf.create_dataset('X_test', data=X_test)
